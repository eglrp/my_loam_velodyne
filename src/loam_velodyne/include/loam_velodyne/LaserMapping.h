...
private:
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_filtered (pcl::PointCloud<pcl::PointXYZI>);
  pcl::PassThrough<pcl::PointXYZI> pass;    //设置滤波对象

  pcl::PointCloud<pcl::PointXYZI>::Ptr _laserCloudSurround;
  pcl::PointCloud<pcl::PointXYZI>::Ptr _laserCloudSurroundDS;     //降采样
  ...
  //PointXYZI 成员：float x,y,z,intensity; 这是一个简单的只有XYZ信息加上强度信息的类型
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> _laserCloudSurfArray;
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> _laserCloudCornerDSArray;  ///降采样
  std::vector<pcl::PointCloud<pcl::PointXYZI>::Ptr> _laserCloudSurfDSArray;    ///降采样
